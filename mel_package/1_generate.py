...
def process_reaction_worker(worker_args: tuple) -> None:
    ...
    if not worker_args:
        return
    reaction_id, reaction_table, config= worker_args
...
def process_reaction_worker_safe(args):

    reaction_id, reaction_table, config = args
    try:
        process_reaction_worker(args)  # твій існуючий код
        return {
...
def main(config):
    ...
    if config.get('consequent_processing', False):
        results = []
        for reaction_id in reaction_ids_to_process:
            ...
            results.append(process_reaction_worker_safe((reaction_id, reaction_table, config)))
    else:
        ...
        with Pool(processes=config['n_processes']) as pool:
            results = pool.map(process_reaction_worker_safe, multiprocessing_arguments)


    log_processing_report(results, config['id_mapper_path'], logger)
